...
interprete = tf.lite.Interpreter(model_path=modelo_tflite)
...
# Etiquetas de clases
etiquetas_clases = {0: "cutter", 1: "Pelota"}
...
while cap.isOpened():
    ...
    imagen = imagen.astype(np.float32)  # Convertir a FLOAT32
    imagen = imagen / 255.0
    imagen = np.expand_dims(imagen, axis=0)  # Agregar la dimensión del lote
    imagen = np.expand_dims(imagen, axis=-1)  # Agregar la dimensión de canales
